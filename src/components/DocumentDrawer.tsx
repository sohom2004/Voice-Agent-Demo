  ...
  const handleFiles = async (fileList: FileList | null) => {
    ...
    for (let i = 0; i < fileList.length; i++) {
      ...
      try {
        ...
        let docType = 'text';
        if (file.name.endsWith('.md')) docType = 'markdown';
        else if (file.name.endsWith('.json')) docType = 'json';
        else if (file.name.endsWith('.csv')) docType = 'csv';
        else if (file.name.endsWith('.js') || file.name.endsWith('.ts') || file.name.endsWith('.py')) docType = 'code';
        ...
        const docItem: DocumentFile = {
          ...
          suggestedQuestions: [
            `What is the main purpose described in ${file.name}?`,
            `Can you summarize the key points in ${file.name}?`,
            `Are there any action items or specs in ${file.name}?`
          ]
        };

        newDocs.push(docItem);

        // Call backend in background to extract rich summary and questions
        fetch('/api/analyze-document', {
          ...
          .then(res => res.json())
          .then(data => {
            if (data.summary || data.suggestedQuestions) {
              docItem.summary = data.summary || docItem.summary;
              docItem.suggestedQuestions = data.suggestedQuestions || docItem.suggestedQuestions;
            }
          })
          .catch(() => {});
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          >
            <input
              ...
              id="doc-file-input"
              multiple
              accept=".txt,.md,.json,.csv,.py,.js,.ts,.tsx,.html,.css,.yaml,.yml"
              className="hidden"
            ...
            >
              ...
              <p className="text-xs opacity-50">
                Supports Markdown, TXT, JSON, CSV, TypeScript & Python
              </p>
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2.5 min-w-0">
                        <span className="w-8 h-8 rounded-lg bg-emerald-500/20 border border-emerald-400/20 flex items-center justify-center text-emerald-400 text-xs font-mono">
                          {doc.type === 'markdown' ? 'MD' : doc.type === 'json' ? 'JSON' : 'TXT'}
                        </span>
                        <div className="min-w-0">
                          ...
                          <p className="text-[10px] opacity-40">
                            {Math.round(doc.size / 1024)} KB • Loaded
                          </p>
                        </div>
                      </div>
